...
確定案（[03章](./03-finalists.md)）を、**JDCC（日本データセンター協会）を起点に、
...
本章は、現行コア設計を**外部基準と照合**し、初版スコープの外にある領域も含めて可視化する。
対象は IPAM・0U/busway・discovery 来歴・DC給電/OCP に加え、耐震・災害・法定点検・物理セキュリティ記録・
運用ライフサイクル・サブシステム別可用性クラス・カーボン/水報告までを含む。
...
> ⚠️ **最重要の但し書き — 「DC に必要」≠「DCIM に必要」**：
> 本章は、DC 事業者がどこかで必要とするデータを広く拾っている。
> ただし、その多くは **DCIM が保持すべきデータではない**。
> ESG/サステナ基盤・CMMS/設備管理・PSIM/入退管理・ITSM・ITAM/ERP が本体を持ち、
> DCIM は参照キーや API で連携するだけでよい領域が多い。
>
> スキーマに抱え込むと、製品が CMMS/ESG/ITSM の劣化コピーに膨らむ。
> **§0.5 のスコープ判定が本章の結論**であり、以下の各 G の詳細はその判定の根拠資料として読む。

**スコープ判定の凡例**: ✅ **DCIM コア**（スキーマで保持） / 🔶 **連携接点**（参照キー・属性だけ持ち、本体は外部） / ❌ **対象外**（別システムが保持。API 連携に留める）
...
各クラスタは内部で**コア/連携/対象外が混在**する。
したがって「クラスタ単位で採否」を決めず、§0.5 の項目単位で判定する。

| # | クラスタ | クラスタ内の主たる判定 | 代表ギャップ |
|---|---------|---------------------|------------|
| G1 | **信頼性・保護の多次元等級** | 🔶 連携（冗長モデルは既存コア／多次元等級は属性） | 単一 `redundancy` enum では「電気=Tier3／機械=Tier2」を表現不能 |
| G2 | **日本固有：耐震・災害・立地** | 🔶 連携（building の属性として） | 建屋/ラックの耐震構造・PML・洪水/活断層ハザードが皆無 |
...
## 0.5. ★スコープ判定 — DCIM が保持すべきもの（本章の結論）

### 境界線の原則
**DCIM のコア = 物理インフラのモデル ＋ テレメトリ ＋ キャパシティ ＋ 構内接続**。
隣接領域（ESG/サステナ・CMMS/設備保全・PSIM/入退管理・ITSM・ITAM/ERP）は **DCIM が持つのでなく連携する**。
DCIM 側にはキーや属性だけを保持し、本体は外部システムが持つ。
判断基準は一貫して **「DCIM が一次的に測る・配置する・容量計算する対象か？」**。
...
| ✅ **コア** | TIA-942 構造化配線スペース（MDA/HDA/ZDA/EDA）と cross-connect | G6 | 構内配線トポロジー＝既存 `cable`/`connection` の正当な拡張 |
| ✅ **コア** | キャリア回線・cross-connect・MMR | G5-P6 | コロ DC の接続管理は DCIM 中核。※IP/VLAN=IPAM は対象外で正しい |
...
| 🔶 **連携** | 可用性/保護/粒度の**等級を属性として**（`room.tier` 等の少数列） | G1 | 冗長モデル(SPOF検出)は既にコア。等級は営業・容量計画用メタ。多スキーム `resilience_rating`＋認証イベント表は作り込み過剰 |
| 🔶 **連携** | 耐震構造・PML・立地ハザードを **building の属性として** | G2 | 日本市場なら数列追加は安い。ただし本質は BCP/不動産/立地データ。ハザード管理モジュールまでは作らない |
| 🔶 **連携** | 物理 MAC（Move/Add/Change）の作業ワークフロー | G5-P2 | 物理移設の作業管理は DCIM 寄り。ただしフルな変更管理プロセスは ITSM。ITSM チケット ID との紐付けに留める |
| 🔶 **連携** | 機器の保証/EOL/EOSL・保守契約の SLA 属性 | G5-P3 | 機器マスタの属性として持つのは妥当。点検「実施」管理は CMMS 寄り |
...
| ❌ **対象外** | 入退室イベント・バッジ・監視カメラ | G5-P1 | 物理セキュリティ（PSIM/入退管理システム）が保持。DCIM はゾーンを参照するだけ |
...
### 連携接点の設計（DCIM が保持しないものをどう扱うか）
❌ 項目は**捨てる**のではなく、**DCIM 側に「外部キー（参照ID）と最小限の状態」だけを置く**:
- ESG 基盤へ → DCIM は `measurement`/`series`（電力量・水・排熱）を**読み取り API/ビューで公開**。係数・申告は受け手が持つ。
- CMMS へ → `equipment` に **`external_cmms_id`** を持たせ、点検実施は CMMS。DCIM は保守*予定*（次回点検日）の表示参照のみ。
- PSIM へ → `location` のゾーンに **保護クラス属性**（🔶 G1）は持つが、入退室イベントは PSIM の参照キーで連携。
- ITSM へ → アラート発火時に **ticket_ref** を記録。インシデント本体は ITSM。
- ERP/ITAM へ → `equipment` に **`asset_tag`/`external_asset_id`**（既存）。財務属性は ERP 側。

> **結論**：6 クラスタのうち DCIM が保持すべきなのは **G6 全部・G5-P6・発電機キャパシティ(G3一部)・水/排熱計量(G4一部)・冗長/Tier 属性(G1)** に限る。
> 残り（炭素会計・法定点検・入退室・ITSM・財務）は**連携接点（参照キー）に格下げ**する。これにより製品は「DCIM の本分」に収まり、
> 設計ゴール「可変性 × ドメイン制約の厳格さ」を保ったまま肥大化を防げる。

> **横断の発見（コア採用分のみ）**：保持する範囲に絞っても、「**人・組織マスタ（`vendor`/`carrier`）**」と
> 「**汎用台帳（係数 `factor_value` 等）**」の共通基盤は有効。個別テーブルを乱立させず共通受け皿を先に決める方針は維持する。

---

## G1. 🔶連携 信頼性・保護の「多次元等級」— 単一 redundancy enum の限界
...
### 何が足りないか
現設計の可用性表現は `connection.redundancy`（`A|B|N|N+1|2N` の単一 enum・**電力/冷却エッジ単位**）と、
...
**律速サブシステムをクエリできない**。施設 Tier 単一値と `connection` 上の冗長情報が二重化する。

### スキーマ案
```text
resilience_rating(
  scope_type {facility|room|panel_equipment|cooling_unit|cage|...}, scope_id,
...
```
- `connection.redundancy` は**実装事実**、`resilience_rating` は**設計等級**として分離する。
- JDCC のマルチティアは `scope_type='room'` 行で受ける（`room.fs_tier` を直接列に持つ簡易版でも可）。
- GN（粒度レベル）は [10章](./10-measurement-scope-derived-metrics.md) の pPUE 算出境界（`measurement_scope`）のメタとして持たせ、PUE の準拠性を記録。
...
```
`scheme`/`award_type` は**ルックアップ表化**する。基準は版を重ねる（FS Ver.2.3 → 3.0 等）ため CHECK 列挙は不可。
...
現設計の Building/Site は空間アンカーのみで**これらを一切持たない**。日本市場向けには未整理のギャップ。
...
- 建築設備の**耐震クラス S/A/B**・設計用標準震度 Ks は `equipment` 補助列（重要機器=クラス S 要求）として持てる。
...
| **冷凍機** | 高圧ガス保安法（法定冷凍能力トンで第一種/第二種区分） | 冷凍能力(トン)・冷媒・製造者区分・許可番号 | `refrigeration_unit(equipment_id, legal_refrig_capacity_ton, refrigerant, hpgas_class, permit_no)` |
| **非常用発電機** | — BCP 中核 | 定格 kVA・燃料種別・**無給油連続運転可能時間（72h 等）** | `generator(equipment_id, rated_kva, fuel_type, runtime_target_h)` |
...
> ❌ **排出係数レジストリ・Scope1/2/3 炭素会計・省エネ法/温対法/EED の申告**は ESG/サステナ基盤が保持する別製品領域。
...
現設計は PUE/pPUE・電力量・時系列までは持つ（[10章](./10-measurement-scope-derived-metrics.md)）が、
...
| **WUE**（水使用効率） | 30134-**9** / 4-9 | 冷却**補給水(make-up)・蒸発・ブローダウン**の水メータ、WUE_source は水係数 | `equip_kind.resource_class` に water 区別 ＋ `makeup_water`(L) point |
...
| **ERF**（排熱再利用係数） | 30134-**6** / 4-6 | 外部供給した**排熱の熱量メータ（地域熱供給等）** | `connection` に `medium='heat'` の outbound を許可＋輸出メータ raw series |
| **ITEEsv / ITEUsv** | 30134-**4/5** | サーバの**定格性能・効率**と「server」フラグ（分子が計算不能） | `equipment_type.rated_peak_perf`, `rated_efficiency` ＋ `equip_kind` の server サブツリー |
...
通貨・kWh・排出量の派生は [04章 UC-1](./04-validation-queries.md) と同じく `numeric`。
...
> 🔶 **P2 物理 MAC ワークフロー**（ITSM チケットへの参照に留める）・**P3 保証/EOL/EOSL 属性**（機器マスタの列）は連携接点。
...
現設計の運用情報は `maintenance_contract`（vendor/番号/期間）と `equipment`（status/installed_on/asset_tag/serial）のみ。
...
work_order_target(work_order_id, entity_type {equipment|cable|rack|connection}, entity_id)   -- 影響資産
```

### P3. 予防保全 + 保証/EOL/EOSL（現 contract が薄い）
発電機/UPS/CRAC の定期点検スケジュールと実施履歴、保証期限、**EOL/EOSL（保守終了）**、SLA（オンサイト応答時間）が持てない。
```text
pm_schedule(equipment_id|equip_kind, task, interval_months, next_due, vendor_id, mop_id)
pm_record(schedule_id, performed_at, work_order_id, result, findings)
asset_warranty(equipment_id, warranty_end, eol_date, eosl_date, support_level)
...
```text
procurement(id, po_number, vendor_id, ordered_at, received_at, cost, ownership {owned|leased}, lease_end)  -- equipment.procurement_id
asset_financial(equipment_id, acquisition_cost, depreciation_method, useful_life_months, in_service_date, book_value)
asset_disposal(equipment_id, disposed_at, method {resale|recycle|destroy}, certificate_ref, proceeds)
inventory_audit(id, location_id, date) / inventory_audit_line(audit_id, equipment_id, expected, found, discrepancy)
```
...
幅・ユニット高さ・レール規格が無いと OCP/ETSI 機器の**搭載互換性・U 重なり計算（[03章 L1](./03-finalists.md) 占有U行）が破綻**する。
```text
...
equipment_type: mount_standard, width_in   -- 搭載適合を CHECK
```

### OCP DC 給電（48V busbar / power shelf）
現 [03章 L6](./03-finalists.md) は AC 給電を中心にした `connection` / `power_connection` モデル。OCP は
**垂直 DC busbar 48V（400/700/1400A, 18kW級）+ power shelf** が一級要素で **outlet 概念が無い**。
初版コアで薄い DC 給電/OCP の扱いを、ここで具体化:
```text
equip_kind に power_shelf|busbar|rectifier|bbu_shelf を追加
power_connection: power_type {ac|dc}, dc_voltage {48|...}, busbar_rating_a {400|700|1400}
-- busbar→OU位置 給電は connection medium='elec'、A/B 系は busbar 系統で表現
```
48V 通信局給電・OCP busbar(48/54V) は、電力を**媒体だけでなく給電方式**まで拡張して扱う。
...
-- cabling は connection medium='data'/'fiber' 区間として載せる
```
...
| 優先 | 項目 | 対応 |
|------|------|------|
| P0 | G6 ラック規格・OCP/DC給電 | `rack.mount_standard`/`mount_width_in`/`unit_height_mm`・`rack_unit_occupancy.unit_type{U\|OU}`・`power_connection.power_type{ac\|dc}`/`busbar_rating_a`・`equip_kind` に power_shelf/busbar |
...
| P2 | G4 水・排熱の計量 | `equip_kind.resource_class` に water・`connection medium='heat'` outbound・補給水/排熱輸出メータの series |

### 🔶 連携 — 属性・参照キーだけ持つ（最小実装）
| 優先 | 項目 | 対応 |
|------|------|------|
| P1 | G1 信頼性/保護/粒度の等級 | `room.tier`・`location.protection_class`・`measurement_scope.granularity_level` 程度の**少数列**。多スキーム表は作らない |
...
| P2 | G5-P3 保証/EOL/EOSL・SLA 属性 | `equipment`/`maintenance_contract` への属性追加（`eol_date`/`eosl_date`/`response_sla`） |

### ❌ 対象外 — 連携キーのみ（DCIM では実装しない）
| 項目 | 連携先 | DCIM 側に置くもの |
|------|--------|-----------------|
| G4 炭素会計・省エネ法/温対法/EED 申告 | ESG/サステナ基盤 | `measurement`/`series` の**読み取り公開ビュー**のみ |
| G3 法定点検記録・有資格者・報告 | CMMS/施設管理 | `equipment.external_cmms_id`・次回点検日の表示参照 |
| G5-P1 入退室・バッジ・カメラ | PSIM/入退管理 | ゾーンの保護クラス属性＋ PSIM 参照キー |
| G5-P4 インシデント/SLA/RCA | ITSM | アラート発火時の `ticket_ref` |
| G5-P5 減価償却・調達・簿価 | ITAM/ERP | `equipment.asset_tag`/`external_asset_id`（既存） |
| G5-P7 監査ログ | フレームワーク層 | （DCIM 固有のドメイン機能ではない） |

> **総括**：[03章](./03-finalists.md) がコア設計を定義するのに対し、本章は**外部基準との照合から DCIM の境界線を引く**。
> 当初の網羅調査は「DC に必要なデータ」を 6 クラスタで広く拾ったが、**その多くは DCIM が保持すべきデータではなかった**。
> 結論は **(1) 実装すべき ✅ コアは G6（ラック規格/OCP/構造化配線）・G5-P6（キャリア回線）・発電機キャパシティ・水/排熱計量に限る**、
> **(2) G1 等級・G2 耐震は 🔶 最小列の属性に留める**、**(3) 炭素会計・法定点検・入退室・ITSM・財務は ❌ 連携接点（参照キー）に格下げ**。
> 抱え込む範囲を絞ることで、肥大化を防ぎつつ「可変性 × ドメイン制約の厳格さ」を保てる。
> これが本章の最大の成果。
