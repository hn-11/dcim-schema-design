...
加算的（additive）オプションモジュール**として設計する。
有効化の有無や粒度は、運用プロファイルで切り替える。
これが本章の切り分け方。

コア側は既に**最小のテナント原始要素**を型付きで持つ（[03章 L1/L2](./03-finalists.md)）:
...
管理する設計（[03章 L7](./03-finalists.md) の `equipment_demand.load_strategy='contracted'` / `contracted` 列）を、
...
コアは既に `tenant` を持つので、**モジュールが足す接点は「NULL 許容の非正規化列 2 つ」だけ**に絞る。
これによりコアの DDL/クエリはテナント有無で変わらず、モジュールを足す/外すが容易になる:

```sql
-- 03章が既にコアに持つテナント原始要素（本章では作らない・再掲）:
--   tenant(id, name, kind IN ('internal','colo_customer'))   -- L1
--   location.tenant_id   -- L1: cage 等、コロ顧客所有の区画
--   equipment.tenant_id  -- L2: 資産の所有者（コロ顧客）
-- 本章モジュールが足す非正規化列は 2 列だけ（すべて NULL 許容・無効時は未使用）
ALTER TABLE series     ADD COLUMN tenant_id bigint;  -- 再課金/RLS 用に非正規化（L5 series 台帳）
ALTER TABLE connection ADD COLUMN tenant_id bigint;  -- 専用接続/給電経路の帰属（任意・L6）
```

> モジュール本体は **別スキーマ `tenancy`** に置くと物理的にも区切れる（`tenancy.space_lease` 等）。
> 本章では簡潔さのため修飾を省く。FK はモジュール有効時のみ張る（無効構成では接点列は単なる未使用列）。
...
```sql
-- コア（03章 L1）のテナント（再掲・最小原始要素・本章では作らない）
-- CREATE TABLE tenant (
...
-- 本章が足す非正規化列に FK（モジュール有効時）
ALTER TABLE series     ADD CONSTRAINT series_tenant_fk
    FOREIGN KEY (tenant_id) REFERENCES tenant(id) ON DELETE SET NULL;
ALTER TABLE connection ADD CONSTRAINT connection_tenant_fk
    FOREIGN KEY (tenant_id) REFERENCES tenant(id) ON DELETE SET NULL;
-- equipment.tenant_id / location.tenant_id の FK はコア（03章 L1/L2）が既に張る
```
...
物理階層から分離した第一級エンティティ。粒度は **排他アーク**（多態回避＝実 FK 化）で表す。
**cage は既存 `location`（`loc_type='cage'`）**に乗るので、配下の集約クエリが
そのまま効く。

> **コロのテナント境界＝cage（EcoStruxure）**: EcoStruxure IT はコロケーションのテナント境界を
> **room 内の cage** としてモデル化する。コア（[03章 L1](./03-finalists.md)）の `loc_type` は `dataCenter`/`cage` を
...
> 契約面（区画単位）にあたる。機器面の契約電力は [03章 L7](./03-finalists.md) の
...
`series.tenant_id`（非正規化）で割るだけ。
共有環境は機器所有で按分し、専用接続/給電経路は `connection.tenant_id` で帰属を持つ。
メトリックの絞り込みは [03章 L3](./03-finalists.md) の**フラット `metric` カタログ**の `code` 列で行う。
旧 3軸 `point_type` は廃止済みで、`series` が `metric_id` を非正規化保持する。

```sql
WITH per_series AS (
    SELECT s.tenant_id, s.series_id,
           sum(d.sum_v) / NULLIF(sum(d.n), 0) AS avg_w,   -- 1d ロールアップ avg=Σsum_v/Σn（[10章](./10-measurement-scope-derived-metrics.md)）
           count(*) AS n_days
    FROM measurement_1d d
    JOIN series s  USING (series_id)
    JOIN metric m  ON m.id = s.metric_id AND m.code = 'active_power'   -- 03章 L3: フラットカタログ
...
> **計測境界は measurement_scope として持つ（[10章](./10-measurement-scope-derived-metrics.md)）**: 個別計量された cage は
> 電力境界としての `measurement_scope` に切り出せる。`series.measurement_scope_id` と `series.tenant_id` は同じ測定流を
...
-- EcoStruxure の contracted power 戦略 ＋ WP-150 の stranded = reserved − actual（[03章 L7](./03-finalists.md)）
SELECT l.tenant_id,
...
機器粒度では同じ突合を [03章 L7](./03-finalists.md) の `equipment_demand`（`load_strategy='contracted'` の
...
erDiagram
    ...
    TENANT ||--o{ EQUIPMENT : "owns (equipment.tenant_id, core L2)"
    TENANT ||--o{ LOCATION : "owns cage (location.tenant_id, core L1)"
    TENANT ||--o{ SERIES : "tenant_id 非正規化"
    TENANT ||--o{ CONNECTION : "dedicated path (tenant_id, nullable)"
    ...
    TENANT {
        bigint id PK
        text name UK
        text kind "internal|colo_customer (core L1)"
    ...
    EQUIPMENT {
        bigint id PK
        bigint tenant_id FK "core link (L2)"
    }
    LOCATION {
        bigint id PK
        bigint tenant_id FK "core link: cage 所有 (L1)"
    }
    SERIES {
        bigint series_id PK
        bigint tenant_id FK "nullable (RLS/billing)"
    }
    CONNECTION {
        bigint id PK
...
1. **コアは最小原始要素のみ**：コア（[03章](./03-finalists.md)）が型付きで持つのは `tenant(id/name/kind)` ＋
   `location.tenant_id` ＋ `equipment.tenant_id`。本章が足す接点は非正規化列 2 つ（`series.tenant_id` / `connection.tenant_id`）のみで、
...
4. **再課金・キャパシティはテナント軸の派生**：既存 UC を `metric.code` ＋ `series.tenant_id` で割るだけ。
   コミット（契約電力）vs 実測で販売余地（stranded）を出し、機器粒度は [03章 L7](./03-finalists.md) の
   `equipment_demand.contracted`（EcoStruxure contracted power 戦略）と対応。per-cage 計測は [10章](./10-measurement-scope-derived-metrics.md) の `measurement_scope` 境界と一致。

> これにより、マルチテナント/コロの重い機構は **「分離可能モジュールとして解決方針あり」** になる。
