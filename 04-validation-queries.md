...
`equipment`（breaker も `equip_kind='breaker'` の行として保持）, `equipment_type`(+`equip_kind_id`), `manufacturer`,
電力チェーン `connection`(medium=elec エッジ) ＋ `power_connection`(CTI サブタイプ・`available_power_w` 生成列),
導出フロービュー `v_equip_flow`（`connection` を再帰展開・`medium.code='elec'` で電力フロー抽出），
メトリックカタログ `metric`（フラット・`code`/`category` で識別）,
`data_point`(+`role`: sensor/sp/cmd), `series`(+`metric_id`,`equipment_id`,`rack_id`,`location_id`,`measurement_scope_id`),
`measurement`(hypertable), `measurement_1m/1h/1d`(CAGG), `current_value`(+`cur_status`,`alert_state`),
`capacity_budget`（排他アーク FK: `location_id`/`rack_id` nullable＋CHECK 1つ）/`equipment_demand`,
...
- **PUE 拡張**: 同じ枠組みで「IT 負荷合計」（`equip_kind.power_class='it'` の機器配下）と「施設総入力」を集計し
  比を取る。派生 pPUE は `metric.is_derived=true`（`code='ppue'`）× 計測スコープ集約で持つ（[10章](./10-measurement-scope-derived-metrics.md)）。
...
```sql
-- v_equip_flow の想定列: from_equipment_id, to_equipment_id, connection_id, medium_id, redundancy
-- （connection WHERE medium_id = (SELECT id FROM medium WHERE code='elec') を再帰展開したビュー）
WITH RECURSIVE upstream AS (
    -- 起点機器の直上流（v_equip_flow で to=自分 を辿る）
    SELECT vf.from_equipment_id          AS up_equip_id,
           vf.redundancy                 AS leg,   -- A|B|N+1|2N|none（connection.redundancy）
           1                             AS depth,
...
- **勘所**:
  - **`v_equip_flow` ビュー**: `connection WHERE medium_id = (SELECT id FROM medium WHERE code='elec')` を
    `from_equipment_id/to_equipment_id` で再帰展開したビュー。`connection` が電力チェーンの真実源なので、
    並行する汎用グラフを手で維持する必要がない（[03章 L6](./03-finalists.md)）。
    A/B 系統の識別は `connection.redundancy`（`A|B|N+1|2N|none`）をそのまま取得。
  - **電力・冷却の分離**: 冷却フローが必要な場合は同じ `connection` を `medium.code='chilled_water'`
    （または `'air'`）でフィルタした別ビューを辿る。基底テーブルは共通で媒体コードだけ変える。
  ...
  - **breaker の物理詳細**: ブレーカは専用表を作らず、`equip_kind.code='breaker'` の `equipment` 行として持つ。
...
- **勘所**:
  ...
  - **`redundancy_group` との照合**: 冗長グループ（`2N`/`N+1`）に所属しながら独立ソース < 2 の機器が
    設計違反。`rg.topology`（意図）と `independent_sources`（現実）を並べ、設計意図と現物構成のズレを可視化する。
...
SELECT
    ...
    -- 排他アーク CHECK により rack_id は自動的に NULL が保証される
...
- **勘所**:
  ...
  - **排他アーク FK**: `capacity_budget` の検索は `location_id = :room_id`（CHECK 制約により
    `rack_id` は DB が NULL を保証・旧 `scope`/`scope_id` 多態キーを廃止・[03章 L7](./03-finalists.md)）。
...
-- capacity_budget のスコープは location/rack のみ。回路容量は power_connection.available_power_w が担う（[03章 L6](./03-finalists.md)）
```

- **勘所**:
  - **排他アーク FK**: `capacity_budget` のスコープ識別は `rack_id = :rack_id`（`location_id IS NULL` で排他を明示。
    CHECK 制約 `num_nonnull(location_id, rack_id) = 1` が DB でも保証。location スコープは `location_id = :room_id`
    に替えれば部屋・フロア・建屋単位の容量予算も同じ表で扱える・[03章 L7](./03-finalists.md)）。
  - **回路容量は `power_connection.available_power_w`**: ラック→PDU 間などの回路容量は
    `power_connection.available_power_w`（V×A×util×√3 の生成列）が持つ。`capacity_budget` に
...
| **電力/冷却トレース** | `v_equip_flow`（`connection` medium=elec の再帰展開ビュー）＋ `WITH RECURSIVE`（PG/MySQL 両対応）。`connection` が真実源なので並行グラフ不要。A/B は `connection.redundancy`、SPOF も同じビューで。本番は path キャッシュ |
...
| **容量 KPI** | `capacity_budget`（排他アーク FK: location/rack のみ）× `equipment_demand(load_strategy)` × 実測で `stranded = reserved − actual`。回路容量は `power_connection.available_power_w` が担う。adjusted_nameplate/predicted で死蔵を圧縮 |
| **電気代/PUE** | 日次ロールアップ（長期 retention）で過去月も即答。avg は `Σsum_v/Σn`、kWh counter があれば差分でより厳密。pPUE は `metric.is_derived=true`（`code='ppue'`）× 計測スコープ（[10章](./10-measurement-scope-derived-metrics.md)） |
