...
スキーマ全体は [03章](./03-finalists.md)、移植性（LCD）方針は [09章](./09-portability.md)、
派生メトリック（pPUE・計測スコープ）は [10章](./10-measurement-scope-derived-metrics.md)。

前提テーブル（[09章](./09-portability.md)の LCD 版）: `location`(+`location_closure`), `rack`(+`rack_unit_occupancy`),
`equipment`（breaker も `equip_kind_name='breaker'` の行として保持）, `equipment_type`(+`equip_kind_id`), `manufacturer`,
電力チェーン `connection`(medium=elec エッジ) ＋ `power_connection`(CTI サブタイプ・`available_power_w` 生成列),
導出フロービュー `v_equip_flow`（`connection` を再帰展開・`medium.medium_name='elec'` で電力フロー抽出），
メトリックカタログ `metric`（フラット・`metric_name`/`category_id` で識別）,
`data_point`(+`point_role`: sensor/sp/cmd), `series`(+`metric_id`,`equipment_id`,`rack_id`,`location_id`,`measurement_scope_id`),
`measurement`(hypertable), `measurement_1m/1h/1d`(CAGG), `current_value`(+`telemetry_status`,`alert_level`),
`location_capacity` / `rack_capacity`（種別テーブル分割）/`equipment_demand`,
`threshold(series_id)`, `redundancy_group`/`redundancy_member`。

> **メトリックの意味づけは `metric` フラットカタログで識別**（旧 `point_type` ＋ 3軸小マスタ廃止・[03章 L3](./03-finalists.md)）。
> `metric` は `metric_name`（`active_power`|`rack_inlet_temp`|`chw_supply_temp` 等）/ `category_id` /
> `unit` / `metric_datatype` / `metric_value_kind` / `agg_function` / `is_derived` を1行に持つ。
> - **有効電力系列** = `m.metric_name = 'active_power'`
> - **吸気温度系列** = `m.metric_name = 'rack_inlet_temp'`（または `metric_category.category_name = 'temperature'` で温度系全体）
>
...
WITH target_series AS (
    ...
    JOIN metric m            ON m.id = s.metric_id AND m.metric_name = 'active_power'  -- ★ フラットカタログ: metric_name 直指定
...
- **勘所**:
  - **`metric.metric_name` 直指定**: 「有効電力」は `m.metric_name = 'active_power'` の1 JOIN で完結（旧 `quantity`/`phenomenon`
    への `haystack_symbol` 2軸 JOIN は不要）。`metric` 行が単位を1つ持つので、集計対象が物理的に W で
    あることを DB が保証する（[03章 L3](./03-finalists.md)）。
  ...
  - 積算電力量メトリック（`metric.metric_value_kind='counter'`, `metric_name='active_energy'` 等）があれば差分計算がより正確。
...
- **PUE 拡張**: 同じ枠組みで「IT 負荷合計」（`equip_kind.power_class='it'` の機器配下）と「施設総入力」を集計し
  比を取る。派生 pPUE は `metric.is_derived=true`（`metric_name='ppue'`）× 計測スコープ集約で持つ（[10章](./10-measurement-scope-derived-metrics.md)）。
...
```sql
-- 当該ラックの吸気温度 series（metric.metric_name で識別）
WITH temp_series AS (
    SELECT s.series_id
    FROM series s
    JOIN metric m ON m.id = s.metric_id AND m.metric_name = 'rack_inlet_temp'  -- ★ metric_name 直指定
...
- **勘所**:
  ...
  - `series.rack_id` は非正規化キー（空間集約用）。ラック直指定の読み出しは追加 JOIN なしで効く。
    意味（吸気温度）の絞り込みは `metric.metric_name = 'rack_inlet_temp'` 1 JOIN だけ（小マスタはキャッシュ常駐）。
    温度系全体を取りたい場合は `metric_category.category_name = 'temperature'`（横断グルーピング）。
...
WITH occupied AS (
    ...
    WHERE rack_id = :rack_id AND rack_side = 'full' AND rack_face = :rack_face
),
...
- **勘所**:
  - 実際の搭載時は `rack_unit_occupancy` への INSERT が**複合主キー (`rack_id, rack_side, rack_face, u`)**で
...
`connection`（`medium_id` で電力フローに絞った有向エッジ）を真実源とし、`v_equip_flow`（導出ビュー）が
equipment 間の電力エッジを展開する。汎用リンク用の別グラフや、並行する別グラフは持たない。
下流が上流を指す向きなので、`v_equip_flow.to_equipment_id = :equipment_id` を起点に遡る。

```sql
-- v_equip_flow の想定列: from_equipment_id, to_equipment_id, connection_id, medium_id, feed_label
-- （connection WHERE medium_id = (SELECT id FROM medium WHERE medium_name='elec') を再帰展開したビュー）
WITH RECURSIVE upstream AS (
    -- 起点機器の直上流（v_equip_flow で to=自分 を辿る）
    SELECT vf.from_equipment_id          AS up_equip_id,
           vf.feed_label                 AS feed_label,   -- A|B|N|N+1|2N|none（connection.feed_label）
           1                             AS depth,
    ...
    SELECT vf.from_equipment_id, u.feed_label, u.depth + 1,
...
SELECT u.feed_label, u.depth, u.up_equip_id,
       ek.equip_kind_name,
       u.path
FROM upstream u
...
ORDER BY u.feed_label, u.depth;
-- equip_kind.equip_kind_name が ups/switchgear/utility 等（v_equip_flow に上流を持たない）行が系統の頂点
```

- **勘所**:
  - **`v_equip_flow` ビュー**: `connection WHERE medium_id = (SELECT id FROM medium WHERE medium_name='elec')` を
    `from_equipment_id/to_equipment_id` で再帰展開したビュー。`connection` が電力チェーンの真実源なので、
    並行する汎用グラフを手で維持する必要がない（[03章 L6](./03-finalists.md)）。
    A/B 系統の識別は `connection.feed_label`（`A|B|N|N+1|2N|none`）をそのまま取得。
  - **電力・冷却の分離**: 冷却フローが必要な場合は同じ `connection` を `medium.medium_name='chilled_water'`
    （または `'air'`）でフィルタした別ビューを辿る。基底テーブルは共通で媒体名だけ変える。
  - インデックス `connection (to_equipment_id)` および `(from_equipment_id)` で各ホップが定数時間。
  - `path` 配列でサイクル検出、`depth < 50` で暴走防止。本番は end-to-end を path キャッシュ表に固める。
  - **breaker の物理詳細**: ブレーカは専用表を作らず、`equip_kind.equip_kind_name='breaker'` の `equipment` 行として持つ。
    親盤は `equipment.parent_equipment_id`、盤内位置は `panel_position`、回路定格は `rated_a` で取得する。
...
`redundancy_group`（設計意図側）と突合することで、冗長設計の意図と現物構成のズレを可視化できる。
...
-- redundancy_group の意図（2N/N+1 等）と照合：冗長グループに属しているのに SPOF なら設計違反
SELECT se.equipment_id,
       se.independent_sources,
       rg.name     AS redundancy_group,
       rg.redundancy_topology AS intended_topology     -- 2N なら独立ソース=2 が必要
FROM spof_equip se
LEFT JOIN redundancy_member rm ON rm.equipment_id = se.equipment_id
LEFT JOIN redundancy_group rg  ON rg.id = rm.group_id AND rg.infra_domain = 'power'
ORDER BY se.equipment_id;
```

- **勘所**:
  - **`v_equip_flow` ベース**: `connection`（medium=elec）を再帰展開した導出ビューのみで SPOF 検出が完結。
    ソースの一意性は `count(DISTINCT up_equip_id)` で判定（`FILTER` 不要なのでそのまま MySQL でも動く）。
  - **`redundancy_group` との照合**: 冗長グループ（`2N`/`N+1`）に所属しながら独立ソース < 2 の機器が
    設計違反。`rg.redundancy_topology`（意図）と `independent_sources`（現実）を並べ、設計意図と現物構成のズレを可視化する。
...
- 同種の監視ビュー（いずれもサービス層）:
  - **回路負荷超過**: `power_connection.available_power_w`（V×A×util×√3 の生成列）に対し、当該 connection
    配下機器の `equipment_demand` / 実測 draw 合計が超過していないか。
...
SELECT
    ...
    (SELECT b.rated
       FROM location_capacity b
       JOIN capacity_dimension d ON d.id = b.dimension_id
      WHERE b.location_id = :room_id
        AND d.dimension_name = 'power_w')       AS rated_w,
    round(100.0 * sum(cv.value) /
      (SELECT b.rated
         FROM location_capacity b
         JOIN capacity_dimension d ON d.id = b.dimension_id
        WHERE b.location_id = :room_id
          AND d.dimension_name = 'power_w'), 1) AS utilization_pct
FROM current_value cv
JOIN series s ON s.series_id = cv.series_id
JOIN metric m ON m.id = s.metric_id AND m.metric_name = 'active_power'  -- ★ metric_name 直指定
JOIN location_closure lc ON lc.descendant_id = s.location_id AND lc.ancestor_id = :room_id
WHERE cv.telemetry_status = 'ok';  -- ★ down/fault/stale を総和から除外

-- ホットスポット: 部屋配下で critical 状態 or 高温の機器
SELECT s.rack_id,
       max(cv.value) FILTER (WHERE m.metric_name = 'rack_inlet_temp') AS max_inlet_temp,
       count(*)      FILTER (WHERE cv.alert_level = 'critical') AS critical_count
FROM current_value cv
JOIN series s ON s.series_id = cv.series_id
JOIN metric m ON m.id = s.metric_id
JOIN location_closure lc ON lc.descendant_id = s.location_id AND lc.ancestor_id = :room_id
GROUP BY s.rack_id
HAVING count(*) FILTER (WHERE cv.alert_level = 'critical') > 0
    OR max(cv.value) FILTER (WHERE m.metric_name = 'rack_inlet_temp') > 27  -- ASHRAE 推奨上限
ORDER BY max_inlet_temp DESC NULLS LAST;
```
> `FILTER` は PG 構文。MySQL では `MAX(CASE WHEN m.metric_name = 'rack_inlet_temp' THEN cv.value END)` /
> `COUNT(CASE WHEN cv.alert_level = 'critical' THEN 1 END)` に置換（方言差・[09章 9.5](./09-portability.md)）。

- **勘所**:
  - **`telemetry_status` と `alert_level` の使い分け**: `current_value` は**収集状態**（`telemetry_status`:
    ok/down/fault/disabled/stale）と**アラート状態**（`alert_level`: normal/warning/critical）を別列で持つ。
    総電力は「健全に取れている値だけ」を足すため `telemetry_status='ok'` で絞り、ホットスポットは
    `alert_level='critical'` で拾う。stale を total に混ぜない（過大/過小評価防止）のが要点。
  - **`metric.metric_name` で意味を絞る**: 「有効電力」は `m.metric_name = 'active_power'`、「吸気温度」は
    `m.metric_name = 'rack_inlet_temp'`（または `metric_category.category_name = 'temperature'`）。旧 `quantity`/`phenomenon` 2軸 JOIN 不要。
    ホットスポット側は全 series を1つの JOIN で取り込み、`FILTER`（MySQL では `CASE WHEN`）で metric 種類を切り分ける。
  - **種別テーブル分割**: 部屋スコープの容量定格は `location_capacity` を、ラックスコープは `rack_capacity` を参照する。
    スコープごとに専用テーブルを持つことで FK 整合性を DB が担保する（旧 `scope`/`scope_id` 多態キーを廃止・[03章 L7](./03-finalists.md)）。
  - `current_value` は PK 1 行参照で最速。ingest 時に UPSERT + `alert_level` 評価（`threshold.series_id` を直接引き、
    hysteresis でフラッピング抑制）。
  - 空間集約は `series.location_id` を `location_closure` で JOIN（`ltree` 不使用・全エンジン同形）。
  - 使用率は `location_capacity.rated` / `rack_capacity.rated` 対比。`stranded = reserved - actual` を出せば過剰プロビジョニング検出（UC-7）。

---

## UC-7. キャパシティ：ラックの 定格 vs 予約 vs 実測（死蔵容量）

**狙い**: `rack_capacity`（定格・冗長予備）× `equipment_demand`（推定負荷戦略の予約）× `current_value`（実測）。

```sql
SELECT d.dimension_name, b.rated, b.soft_limit, b.redundancy_reserve,
       r.reserved_sum,
       a.actual_now,
...
LEFT JOIN LATERAL (   -- 予約合計（equipment_demand から。推定負荷戦略適用後の reserved）
    ...
    JOIN equipment_demand ed ON ed.equipment_id = rm.equipment_id AND ed.dimension_id = b.dimension_id
...
LEFT JOIN LATERAL (   -- 実測（current_value の電力合計・健全値のみ）
    ...
    JOIN current_value cv ON cv.series_id = s.series_id AND cv.telemetry_status = 'ok'
    JOIN metric m        ON m.id = s.metric_id AND m.metric_name = 'active_power'  -- ★ metric_name 直指定
    WHERE rm.rack_id = b.rack_id AND rm.valid_to IS NULL  -- 現在配置のみ(Type-2)
      AND d.dimension_name = 'power_w'
) a ON true
WHERE b.rack_id = :rack_id;
-- 容量定格はスコープ種別ごとにテーブル分割（location_capacity / rack_capacity）。
-- 回路容量は power_connection.available_power_w が担う（[03章 L6](./03-finalists.md)）
```

- **勘所**:
  - **種別テーブル分割**: ラックスコープは `rack_capacity`、location スコープは `location_capacity` を参照する。
    スコープごとに専用テーブルを持つため、FK 整合性を DB が担保し `num_nonnulls` CHECK も不要・[03章 L7](./03-finalists.md)。
  - **回路容量は `power_connection.available_power_w`**: ラック→PDU 間などの回路容量は
    `power_connection.available_power_w`（V×A×util×√3 の生成列）が持つ。`rack_capacity` / `location_capacity` に
    breaker スコープは不要で、回路超過チェックは `power_connection` を参照する監視ビューで行う（UC-5 参照）。
  ...
  - 実測の絞り込みも `m.metric_name = 'active_power'` の1 JOIN（単位整合は `metric` 行が保証）。
  ...
  - 「予約合計 ≤ rack_capacity.rated」「rack 内合計 ≤ power_connection.available_power_w」等の**集約制約はサービス層**で担保
...
| 観点 | 検証結果 |
|------|---------|
| **点の意味づけ** | `metric.metric_name`（`active_power`/`rack_inlet_temp` 等）の1 JOIN で型安全に絞れる。`metric` 行が単位を1つ持つので集計対象の物理次元を DB が保証。旧 `point_type`+3軸 JOIN は不要 |
...
| **電力/冷却トレース** | `v_equip_flow`（`connection` medium=elec の再帰展開ビュー）＋ `WITH RECURSIVE`（PG/MySQL 両対応）。`connection` が真実源なので並行グラフ不要。A/B は `connection.feed_label`、SPOF も同じビューで。本番は path キャッシュ |
| **収集状態とアラートの分離** | `current_value.telemetry_status`（健全性）と `alert_level`（しきい値）を別列に。総和は ok のみ、ホットスポットは critical で拾う。stale 混入による誤集計を回避 |
| **集約制約** | SPOF・容量超過・温湿度逸脱は行間集約のため CHECK 不可 → サービス層（移植性のため DB トリガに依存させない）。SPOF は `redundancy_group` の意図と照合 |
| **容量 KPI** | `location_capacity` / `rack_capacity`（種別テーブル分割）× `equipment_demand(load_strategy)` × 実測で `stranded = reserved − actual`。回路容量は `power_connection.available_power_w` が担う。adjusted_nameplate/predicted で死蔵を圧縮 |
| **電気代/PUE** | 日次ロールアップ（長期 retention）で過去月も即答。avg は `Σsum_v/Σn`、kWh counter があれば差分でより厳密。pPUE は `metric.is_derived=true`（`metric_name='ppue'`）× 計測スコープ（[10章](./10-measurement-scope-derived-metrics.md)） |
