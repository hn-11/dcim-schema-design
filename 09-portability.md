...
大胆な設計変更（アーキテクチャの作り直し）が要らない**作りにする。対象は [03章](./03-finalists.md) の確定設計
**Semantic-Typed DCIM**（オントロジは**有界な型付き小マスタ＋CHECK enum**で表し汎用 `def` DAG は持たない、
点を `metric` フラットカタログに正規化（metric 行が単位を直接保持）、電力フローを物理 path（`power_panel→breaker→feed`）から `v_equip_flow` で導出）。
...
4. **集約制約（予約≤定格・SPOF・容量超過・pPUE 算出・連結成分）はサービス層**で実施
   （deferrable/トリガの方言差を避ける）。`location_closure`・占有U行・`room_group_member`、および採用するなら
...
> **L1 の単純化がそのまま LCD の単純化**: 旧案の汎用 `def`/`def_super`/`def_closure`（Haystack `is` の多重継承 DAG を
...
| 再帰 CTE（`equip_logical_link` 上の論理フロートレース・SPOF / `equip_kind` ツリー集約） | ○ | ○(8.0+) | そのまま |
...
| 生成列（`power_feed.available_power_w` 等） | ○ | ○(STORED・5.7+) | **そのまま**（範囲型の生成列だけ避ければ可・9.5） |
...
ただし派生系列の部分 UNIQUE（[10章](./10-room-group-derived-metrics.md)）だけは MySQL 非対応のため代替が要る（9.4）。
...
派生系列の重複防止に部分 UNIQUE が現れる（[10章](./10-room-group-derived-metrics.md)）:

```sql
-- PG なら書ける（MySQL では不可）
CREATE UNIQUE INDEX ON series (room_group_id, metric_id)
    WHERE series_kind = 'derived' AND scope_type = 'room_group';
```

これは「**derived 系列は (部屋グループ × metric) で一意。raw 系列はこの一意制約の対象外**」を表す。
...
1. **生成列の NULL 一意キー（推奨）**: 「条件を満たす時だけ値を持ち、それ以外は NULL」になる STORED 生成列を作り、
   ...
   ALTER TABLE series ADD COLUMN derived_key text
       GENERATED ALWAYS AS (
           CASE WHEN series_kind = 'derived' AND scope_type = 'room_group'
                THEN room_group_id || ':' || metric_id END) STORED;
   CREATE UNIQUE INDEX series_derived_uk ON series (derived_key);   -- raw は NULL=対象外
   ```

2. **派生系列を別表に分離**: `derived_series` を切り出し、そこに素の `UNIQUE (room_group_id, metric_id)` を張る。
...
- **metric が単位を直接保持（trivially portable）**: `metric` 行が `unit` を1列で持ち、`data_point` は
  `metric_id` の単一 FK だけを参照する。旧来の「`point_type (quantity_id, unit_id) → quantity_unit` 複合 FK」は
  廃止済み（[03章L4](./03-finalists.md)）。単位整合は **metric 行が単位を1つ保持**することで成立し、
  複合 FK の方言差・composite UNIQUE の維持も不要になる——標準の単一 FK で完結する。
- **排他アーク FK（`CHECK num_nonnull(...)=1`）**: `capacity_budget`（location/rack/breaker）と `threshold`
  （metric/equipment/location）はスコープを**排他アーク**で持つ（[03章L8/L9](./03-finalists.md)）。
...
- **`v_equip_flow`（導出フロー）**: 電力フローは物理 path（`power_panel→breaker→power_feed`）からの**導出ビュー**
  であり並行グラフを持たない（[03章L7](./03-finalists.md)）。PG では**マテリアライズドビュー**（`REFRESH MATERIALIZED VIEW`）、
  MySQL では通常の**ビュー**か**テーブル＋バッチ更新ジョブ**で代替する——論理モデルの変更は不要。
- **生成列（GENERATED ... STORED）**: `power_feed.available_power_w`（= `V × A × util × √3` を **bigint** にキャストして
  V×A のオーバーフローを回避）をはじめ、生成列は **PG と MySQL 5.7+ の双方がサポート**。STORED 指定で両対応。
  唯一の注意は「**範囲型の生成列**」だけ避けること（範囲型自体が MySQL に無い）。本設計は数値・真偽の生成列のみ
  なので問題なし。
- **`equip_logical_link` / `breaker` / 電力フロートレース**: いずれも**普通の表 + FK + UNIQUE + CHECK** で構成され、
  PG 固有機能は使っていない。`equip_logical_link` や power path 上流トレースは**標準の再帰 CTE**（MySQL 8.0+ 可）。
  `breaker UNIQUE(panel,position)` は標準の複合 UNIQUE。
- **正規表現 CHECK**（`code ~ '^[A-Za-z][A-Za-z0-9_-]*$'` 等、`location.code`/`room_group` に使用）: 演算子が
...
時系列は **his（`measurement`）/ 派生（`derived_measurement`・[10章](./10-room-group-derived-metrics.md)）/
cur（`current_value`）** の 3 表。いずれも Narrow（long format）で、`series_id` だけが越境する（FK なし）。

| 概念 | TimescaleDB | ClickHouse | 移植性 |
|---|---|---|---|
| 生テーブル `measurement`（his） | hypertable(`create_hypertable`) | `MergeTree ORDER BY (series_id, time)` + `PARTITION BY toYYYYMMDD(time)` | **概念一致**（Narrow が効く） |
...
> **生 `measurement` と `derived_measurement` を分ける理由**（[10章](./10-room-group-derived-metrics.md)）: retention は
...
| 区分 | 例 | 載せ替え時 |
|---|---|---|
| **不変（作り直し不要）** | 30+ のリレーショナル表・ER・FK・UNIQUE・CHECK・排他アーク FK（`CHECK num_nonnull(...)=1`）、metric 単位内包（単一 FK）、親参照ツリー（`equip_kind`/`location`/`equipment` の self-FK）と配下集約の閉包 JOIN（`location_closure`/任意の`equip_kind_closure`）、`equip_logical_link` の再帰 CTE トレース、生成列（`available_power_w`）、Narrow の論理形、`series` 台帳、5操作の呼び出し側 | そのまま |
...
| **サービス層** | 集約制約（予約≤定格・SPOF・容量超過・ASHRAE 逸脱・pPUE 算出）、`location_closure`（および採用するなら任意の `equip_kind_closure`）・占有U行・`room_group` 連結成分の維持 | DB 方言に依存しない実装 |
...
> なお L1 から汎用 `def` DAG を外したことで DAG 閉包維持の複雑さがサービス層から消え、移植性の骨格はむしろ
> **より単純で堅牢**になった。派生メトリクス（`derived_measurement`）の追加（[10章](./10-room-group-derived-metrics.md)）も
> この骨格を崩さない。
